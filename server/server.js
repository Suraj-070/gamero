...
io.on('connection', (socket) => {
  ...
  socket.on('wordWordleGuess', ({ roomCode, guess }) => {
    ...
    // Validate — must be a real word
    if (!VALID_WORDS.has(guess.toUpperCase())) {
      socket.emit('invalidWord', { guess });
      return;
    }
